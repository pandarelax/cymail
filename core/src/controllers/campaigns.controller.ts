...
export class CampaignsController {
  ...
  public async update(req: Request, res: Response) {
    try {
      ...
      const result = await sequelize.transaction(async (t: Transaction) => {
        const campaign = await campaignToUpdate.update(req.body, {
          transaction: t,
        });
  ...
  public async launch(req: Request, res: Response) {
    try {
      ...
      const targetMails: TargetEmail[] = targets.map((target: Target) => ({
      ...
      return res
        .status(200)
        .json({ message: "Campaign launched successfully" });
    } catch (error) {
